...
StudioFeestructureDao.prototype.getListByStudioAreaIdList = function(req, idList, callback) {
	// try {
	// 	validator.isDate(params.saleDate);
	// } catch(error) {
	// 	callback(error);
	...
	mysql.query("select * from studio_feestructure where studio_area_id in " + queryString + " and delete_flag = 0", function (err, rows) {
		if (err) {
			callback(err);
			return;
		}
